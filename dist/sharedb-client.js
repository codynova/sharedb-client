...
Doc.prototype._setType = function(newType) {
  ...
  if (newType) {
    ...
    var err = new error(ERROR_CODE.ERR_DOC_TYPE_NOT_RECOGNIZED, 'Missing type ' + newType);
...
function transformX(client, server) {
  ...
  if (server.del) {
    return new error(ERROR_CODE.ERR_DOC_WAS_DELETED, 'Document was deleted');
  }
  if (server.create) {
    return new error(ERROR_CODE.ERR_DOC_ALREADY_CREATED, 'Document already created');
  }
...
Doc.prototype._sendOp = function() {
  ...
  if (!op) {
    var err = new error(ERROR_CODE.ERR_INFLIGHT_OP_MISSING, 'No op to send on call to _sendOp');
    return this.emit('error', err);
  }
...
Doc.prototype.create = function(data, type, options, callback) {
  ...
  if (this.type) {
    var err = new error(ERROR_CODE.ERR_DOC_ALREADY_CREATED, 'Document already exists');
    if (callback) return callback(err);
    return this.emit('error', err);
  }
...
Doc.prototype.del = function(options, callback) {
  ...
  if (!this.type) {
    var err = new error(ERROR_CODE.ERR_DOC_DOES_NOT_EXIST, 'Document does not exist');
    if (callback) return callback(err);
    return this.emit('error', err);
  }
  var op = {del: true};
...
Doc.prototype._opAcknowledged = function(message) {
  if (this.inflightOp.create) {
    ...
    logger_1.warn('Invalid version from server. Expected: ' + this.version + ' Received: ' + message.v, message);
...
var localPresence = LocalPresence;
...
LocalPresence.prototype._callbackOrEmit = function(error, callback) {
  if (callback) return browser$1.nextTick(callback, error);
  if (error) this.emit('error', error);
};

var remotePresence = RemotePresence;
...
var hasNextTick = typeof browser$1 === 'object' && typeof browser$1.nextTick === 'function';
...
var ERROR_CODE$1 = error.CODES;

var localDocPresence = LocalDocPresence;
...
LocalDocPresence.prototype.submit = function(value, callback) {
  if (!this._doc.type) {
    var error = {
      code: ERROR_CODE$1.ERR_DOC_DOES_NOT_EXIST,
...
var types$1 = types.map;



var ERROR_CODE$2 = error.CODES;

// Returns an error string on failure. Rockin' it C style.
var checkOp = function(op) {
  if (op == null || typeof op !== 'object') {
    return new error(ERROR_CODE$2.ERR_OT_OP_BADLY_FORMED, 'Op must be an object');
  }

  if (op.create != null) {
    if (typeof op.create !== 'object') {
      return new error(ERROR_CODE$2.ERR_OT_OP_BADLY_FORMED, 'Create data must be an object');
    }
    var typeName = op.create.type;
    if (typeof typeName !== 'string') {
      return new error(ERROR_CODE$2.ERR_OT_OP_BADLY_FORMED, 'Missing create type');
    }
    var type = types$1[typeName];
    if (type == null || typeof type !== 'object') {
      return new error(ERROR_CODE$2.ERR_DOC_TYPE_NOT_RECOGNIZED, 'Unknown type');
    }
  } else if (op.del != null) {
    if (op.del !== true) return new error(ERROR_CODE$2.ERR_OT_OP_BADLY_FORMED, 'del value must be true');
  } else if (op.op == null) {
    return new error(ERROR_CODE$2.ERR_OT_OP_BADLY_FORMED, 'Missing op, create, or del');
  }

  if (op.src != null && typeof op.src !== 'string') {
    return new error(ERROR_CODE$2.ERR_OT_OP_BADLY_FORMED, 'src must be a string');
  }
  if (op.seq != null && typeof op.seq !== 'number') {
    return new error(ERROR_CODE$2.ERR_OT_OP_BADLY_FORMED, 'seq must be a string');
  }
  if (
    (op.src == null && op.seq != null) ||
    (op.src != null && op.seq == null)
  ) {
    return new error(ERROR_CODE$2.ERR_OT_OP_BADLY_FORMED, 'Both src and seq must be set together');
  }

  if (op.m != null && typeof op.m !== 'object') {
    return new error(ERROR_CODE$2.ERR_OT_OP_BADLY_FORMED, 'op.m must be an object or null');
  }
};

// Takes in a string (type name or URI) and returns the normalized name (uri)
var normalizeType = function(typeName) {
  return types$1[typeName] && types$1[typeName].uri;
};
...
var apply = function(snapshot, op) {
  if (typeof snapshot !== 'object') {
    return new error(ERROR_CODE$2.ERR_APPLY_SNAPSHOT_NOT_PROVIDED, 'Missing snapshot');
  }
  if (snapshot.v != null && op.v != null && snapshot.v !== op.v) {
    return new error(ERROR_CODE$2.ERR_APPLY_OP_VERSION_DOES_NOT_MATCH_SNAPSHOT, 'Version mismatch');
  }

  // Create operation
  if (op.create) {
    if (snapshot.type) return new error(ERROR_CODE$2.ERR_DOC_ALREADY_CREATED, 'Document already exists');

    // The document doesn't exist, although it might have once existed
    var create = op.create;
    var type = types$1[create.type];
    if (!type) return new error(ERROR_CODE$2.ERR_DOC_TYPE_NOT_RECOGNIZED, 'Unknown type');
...
function applyOpEdit(snapshot, edit) {
  if (!snapshot.type) return new error(ERROR_CODE$2.ERR_DOC_DOES_NOT_EXIST, 'Document does not exist');

  if (edit == null) return new error(ERROR_CODE$2.ERR_OT_OP_NOT_PROVIDED, 'Missing op');
  var type = types$1[snapshot.type];
  if (!type) return new error(ERROR_CODE$2.ERR_DOC_TYPE_NOT_RECOGNIZED, 'Unknown type');
...
var transform = function(type, op, appliedOp) {
  ...
  if (op.v != null && op.v !== appliedOp.v) {
    return new error(ERROR_CODE$2.ERR_OP_VERSION_MISMATCH_DURING_TRANSFORM, 'Version mismatch');
  }

  if (appliedOp.del) {
    if (op.create || op.op) {
      return new error(ERROR_CODE$2.ERR_DOC_WAS_DELETED, 'Document was deleted');
    }
  } else if (
    ...
    return new error(ERROR_CODE$2.ERR_DOC_ALREADY_CREATED, 'Document was created remotely');
  } else if (appliedOp.op && op.op) {
    // If we reach here, they both have a .op property.
    if (!type) return new error(ERROR_CODE$2.ERR_DOC_DOES_NOT_EXIST, 'Document does not exist');
 ...
 */
var applyOps = function(snapshot, ops) {
  var type = null;

  if (snapshot.type) {
    type = types$1[snapshot.type];
    if (!type) return new error(ERROR_CODE$2.ERR_DOC_TYPE_NOT_RECOGNIZED, 'Unknown type');
  }
...
var transformPresence = function(presence, op, isOwnOp) {
  ...
  if (!type) return {code: ERROR_CODE$2.ERR_DOC_TYPE_NOT_RECOGNIZED, message: 'Unknown type'};
  if (!util.supportsPresence(type)) {
    return {code: ERROR_CODE$2.ERR_TYPE_DOES_NOT_SUPPORT_PRESENCE, message: 'Type does not support presence'};
  }
  ...
  try {
    ...
    return {code: ERROR_CODE$2.ERR_PRESENCE_TRANSFORM_FAILED, message: error.message || error};
  }
...
var remoteDocPresence = RemoteDocPresence;
...
SnapshotRequest.prototype._handleResponse = function(error, message) {
  ...
  var snapshot$1 = new snapshot(this.id, message.v, message.type, message.data, metadata);

  this.callback(null, snapshot$1);
};

var snapshotVersionRequest = SnapshotVersionRequest;
...
var snapshotTimestampRequest = SnapshotTimestampRequest;
...
Connection.prototype.bindToSocket = function(socket) {
  ...
  socket.onmessage = function(event) {
    try {
      ...
      logger_1.warn('Failed to parse message', event);
      return;
    }

    if (connection.debug) logger_1.info('RECV', JSON.stringify(data));
...
Connection.prototype.handleMessage = function(message) {
  ...
  switch (message.a) {
    ...
    default:
      logger_1.warn('Ignoring unrecognized message', message);
  }
};
...
Connection.prototype._handleBulkMessage = function(err, message, method) {
  if (message.data) {
    ...
    logger_1.error('Invalid bulk message', message);
  }
};
...
Connection.prototype._setState = function(newState, reason) {
  ...
  if (
    ...
    var err = new error(
      ERROR_CODE$3.ERR_CONNECTION_STATE_TRANSITION_INVALID,
...
Connection.prototype.send = function(message) {
  if (this.debug) logger_1.info('SEND', JSON.stringify(message));
...
Connection.prototype._initialize = function(message) {
  if (message.protocol !== 1) {
    return this.emit('error', new error(
      ERROR_CODE$3.ERR_PROTOCOL_VERSION_NOT_SUPPORTED,
      'Unsupported protocol version: ' + message.protocol
    ));
  }
  if (types.map[message.type] !== types.defaultType) {
    return this.emit('error', new error(
      ERROR_CODE$3.ERR_DEFAULT_TYPE_MISMATCH,
      message.type + ' does not match the server default type'
    ));
  }
  if (typeof message.id !== 'string') {
    return this.emit('error', new error(
      ERROR_CODE$3.ERR_CLIENT_ID_BADLY_FORMED,
